...
describe("NameRegister", () => {
  ...
  describe("ReserveName by alice", async () => {
    it("alice reserve name", async () => {
      await nameRegister.connect(alice).reserveName(name);
      ...
      expect(info.reserveExpiresAt).to.equal(
        reserveExpiresAt
      );
  ...
  describe("ReserveName by bob", async () => {
    it("bob can't reserve alice's name", async () => {
      await expect(
        nameRegister.connect(bob).reserveName(name)
  ...
  describe("RegisterName by alice", async () => {
    it("alice register name", async () => {
      ...
      const fee = value.mul(name.length).mul(lengthFactor).div(lengthMultiplier);
    ...
    it("alice reserve name before expired", async () => {
      await expect(
        nameRegister.connect(alice).reserveName(name)
  ...
  describe("Expired Name", async () => {
    ...
    it("alice can't renew", async () => {
      ...
      await expect(
        nameRegister
          .connect(alice)
          .renewName(name, { value: value.toString() })
    ...
    it("bob can reserve", async () => {
      await nameRegister.connect(bob).reserveName(name);
